...
func (a *AuthController) Create(writter http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		response := responses.NotFound{
			HttpError: responses.HttpError{Code: 404, Message: "User does not exist in our database"},
		}
		responseJSON, err := json.Marshal(&response)
		errors.Check(err)
		a.writeResponse(responseJSON, writter, http.StatusNotFound)
		...
		if existingUser.CheckPassword(body["password"]) {
			...
			if err != nil {
				response := responses.ServerError{
					HttpError: responses.HttpError{Code: 500, Message: "Error creating auth token"},
				}
				responseJSON, err := json.Marshal(&response)
				errors.Check(err)
				a.writeResponse(responseJSON, writter, http.StatusInternalServerError)

			} else {
				response := responses.AuthResponseOK{
					Code:    200,
				...
				a.writeResponse(responseJSON, writter, http.StatusOK)
			}

		} else {
			response := responses.Forbidden{
				HttpError: responses.HttpError{Code: 403, Message: "Password not valid"},
			}
			responseJSON, err := json.Marshal(&response)
			errors.Check(err)
			a.writeResponse(responseJSON, writter, http.StatusNotFound)
